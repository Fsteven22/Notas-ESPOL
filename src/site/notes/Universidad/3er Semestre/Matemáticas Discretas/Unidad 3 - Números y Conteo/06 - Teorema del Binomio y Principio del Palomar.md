...
> > Aplicamos el teorema binomial con $a = b = 1$:
> > $$2^n = (1+1)^n = \sum_{k=0}^{n}\binom{n}{k} 1^{n-k} 1^k = \sum_{k=0}^{n}\binom{n}{k} \quad \blacksquare$$
> 
...
La identidad de Pascal permite construir el **triángulo de Pascal**, donde cada entrada es la suma de las dos que tiene encima:
...
> 
> - La fila $n$ contiene los coeficientes $\binom{n}{0}, \binom{n}{1}, \ldots, \binom{n}{n}$.
> - La suma de la fila $n$ es $2^n$ (por la identidad 1).
> - El triángulo es **simétrico**: $\binom{n}{k} = \binom{n}{n-k}$.
...
> 
> **2.** 12 jugadores de basketball (numerados 1–12) colocados alrededor del cuadro central. Demuestre que hay 3 jugadores consecutivos cuya suma es al menos 20. *(resuelto arriba)*
> 
