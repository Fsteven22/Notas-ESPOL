---
{"dg-publish":true,"permalink":"/universidad/2do-semestre/ingles-ii-c1-c2/unit-0-preliminar/01-grammar-review-the-sentence-and-clauses/","dg-note-properties":{}}
---


# 01 - Grammar Review - The Sentence & Clauses

## 🎯 Introducción

> [!info]- 💡 ¿Qué se repasa en esta nota?
>
> Esta nota cubre los fundamentos de la oración en inglés y los tipos de cláusulas. Son la base sobre la que se construyen todos los tipos de oraciones que verás en Inglés II.
>
> | # | Tema |
> |---|---|
> | **1** | What is a sentence? |
> | **2** | Characteristics of a sentence |
> | **3** | Independent vs. Dependent Clauses |
>
> ```mermaid
> graph TD
>     A[The Sentence & Clauses] --> B[What is a sentence?]
>     A --> C[Characteristics]
>     A --> D[Clauses]
>
>     D --> E[Independent Clause<br/>✅ stands alone]
>     D --> F[Dependent Clause<br/>❌ needs support]
>
>     style B fill:#e1f5ff
>     style C fill:#e1ffe1
>     style D fill:#fff4e1
>     style E fill:#e1ffe1
>     style F fill:#ffe1e1
> ```

---

## 📝 What is a Sentence?

> [!note]- 💡 Definición
>
> A **sentence** is a group of words that expresses a **complete thought**. It always has at least a **subject** and a **verb**, and it makes sense on its own.
>
> > 💡 **Analogía práctica:** Una oración es como una historia mínima — siempre hay alguien (sujeto) que hace algo (verbo), y juntos forman una idea completa.
>
> **Ejemplos:**
>
> ```
> ✅ She runs every morning.
>    (Subject = She | Verb = runs | Complete thought ✔)
>
> ✅ The students studied hard.
>    (Subject = The students | Verb = studied | Complete thought ✔)
>
> ❌ Because she was tired.
>    (Incomplete — leaves us asking: "What happened?")
>
> ❌ Running in the park.
>    (No subject — who is running?)
> ```

---

## 🔍 Characteristics of a Sentence

> [!note]- 📋 Las 5 características obligatorias
>
> Every sentence in English must have these five features:
>
> | Característica | Descripción | Ejemplo |
> |---|---|---|
> | **Subject** | Who or what the sentence is about | ***She*** sings beautifully. |
> | **Predicate (Verb)** | What the subject does or is | She ***sings*** beautifully. |
> | **Complete thought** | Makes sense on its own | She sings beautifully. ✔ |
> | **Capitalization** | Starts with a capital letter | ***S***he sings beautifully. |
> | **End punctuation** | Ends with . ? or ! | She sings beautifully**.** |
>
> **Estructura visual:**
>
> ```
> ┌─────────────────────────────────────────────────────┐
> │  [Capital Letter]  SUBJECT  +  VERB  + ...  [. ? !] │
> └─────────────────────────────────────────────────────┘
>
>   She       runs      every morning.
>   ↑         ↑         ↑
>   Capital   Subject   Verb + Complement   Period
> ```

---

## 🔗 Clauses

> [!note]- 🧱 What is a Clause?
>
> A **clause** is a group of words that contains a **subject** and a **verb**. There are two types:
>
> ```mermaid
> graph LR
>     A[CLAUSE] --> B[Independent Clause]
>     A --> C[Dependent Clause]
>
>     B --> B1["✅ Makes complete sense alone<br/>─────────────────<br/>She loves music."]
>     C --> C1["❌ Does NOT make sense alone<br/>─────────────────<br/>Because she loves music..."]
>
>     style B fill:#e1ffe1
>     style C fill:#ffe1e1
>     style B1 fill:#f0fff0
>     style C1 fill:#fff0f0
> ```
>
> ---
>
> ### ✅ Independent Clause
>
> Can stand alone as a complete sentence. It expresses a complete thought.
>
> ```
> ✅ The teacher explained the lesson.
> ✅ I love learning languages.
> ✅ He works every day.
> ```
>
> ---
>
> ### ❌ Dependent Clause
>
> Cannot stand alone — it depends on an independent clause to complete its meaning. It starts with a **subordinating conjunction** (because, although, when, if, since, while, after, before, unless…).
>
> ```
> ❌ Because she was tired...       (¿Qué pasó?)
> ❌ Although it was raining...     (¿Y entonces?)
> ❌ When I finish my homework...   (¿Qué harás?)
> ```
>
> Cuando se combina con una cláusula independiente, la oración queda completa:
>
> ```
> ✅ Because she was tired, she went to bed early.
> ✅ Although it was raining, we went outside.
> ✅ When I finish my homework, I will watch a movie.
> ```
>
> ---
>
> ### 📊 Comparación rápida
>
> | | Independent Clause | Dependent Clause |
> |---|---|---|
> | **¿Tiene sujeto y verbo?** | ✅ Sí | ✅ Sí |
> | **¿Expresa idea completa?** | ✅ Sí | ❌ No |
> | **¿Puede estar sola?** | ✅ Sí | ❌ No |
> | **¿Empieza con subordinante?** | ❌ No | ✅ Generalmente |
>
> > [!tip]- 💡 Truco para identificar cláusulas dependientes
> >
> > Si la frase empieza con palabras como *because, although, when, if, since, while, after, before, unless*, casi siempre es una cláusula dependiente. Pregúntate: **¿tiene sentido sola?** Si la respuesta es no, es dependiente.

---

## 📝 Ejercicios de práctica

> [!example]- ✏️ Ejercicio 1 — ¿Oración completa o incompleta?
>
> Indica si cada grupo de palabras es una oración completa (✅) o incompleta (❌):
>
> 1. The students finished their project.
> 2. Because it was cold outside.
> 3. Running every morning before class.
> 4. She decided to study abroad.
> 5. Although he tried very hard.
> 6. My professor explained the assignment clearly.

> [!success]- ✅ Respuestas — Ejercicio 1
>
> | # | Respuesta | Por qué |
> |---|---|---|
> | 1 | ✅ Completa | Sujeto (*The students*) + verbo (*finished*) + idea completa |
> | 2 | ❌ Incompleta | Cláusula dependiente — *because* introduce una razón pero no hay cláusula principal |
> | 3 | ❌ Incompleta | No hay sujeto — ¿quién está corriendo? |
> | 4 | ✅ Completa | Sujeto (*She*) + verbo (*decided*) + idea completa |
> | 5 | ❌ Incompleta | Cláusula dependiente — *although* introduce un contraste pero no hay cláusula principal |
> | 6 | ✅ Completa | Sujeto (*My professor*) + verbo (*explained*) + idea completa |

> [!example]- ✏️ Ejercicio 2 — Independent o Dependent?
>
> Clasifica cada cláusula como independiente (I) o dependiente (D):
>
> 1. She loves reading novels.
> 2. When the class ends.
> 3. I will call you later.
> 4. Although the exam was difficult.
> 5. They arrived on time.
> 6. Because he forgot his homework.
> 7. The coffee is too hot.
> 8. Unless you study harder.

> [!success]- ✅ Respuestas — Ejercicio 2
>
> | # | Cláusula | Tipo | Por qué |
> |---|---|---|---|
> | 1 | *She loves reading novels.* | **I** | Sujeto + verbo + idea completa |
> | 2 | *When the class ends.* | **D** | *When* es subordinante — no tiene sentido sola |
> | 3 | *I will call you later.* | **I** | Sujeto + verbo + idea completa |
> | 4 | *Although the exam was difficult.* | **D** | *Although* es subordinante — ¿y entonces qué? |
> | 5 | *They arrived on time.* | **I** | Sujeto + verbo + idea completa |
> | 6 | *Because he forgot his homework.* | **D** | *Because* es subordinante — ¿qué pasó como resultado? |
> | 7 | *The coffee is too hot.* | **I** | Sujeto + verbo + idea completa |
> | 8 | *Unless you study harder.* | **D** | *Unless* es subordinante — no tiene sentido sola |

---

## 📚 Glosario de términos difíciles

> [!quote]- 📖 Palabras y conceptos que pueden causar confusión
>
> | Término | Explicación sencilla |
> |---|---|
> | **Subject** | La persona, cosa o idea sobre la que trata la oración. Responde a la pregunta *¿quién?* o *¿qué?* antes del verbo. |
> | **Predicate** | Todo lo que se dice sobre el sujeto. Incluye el verbo y cualquier complemento. |
> | **Complete thought** | Una idea que no deja preguntas sin responder. Si al leerla piensas "¿y qué pasó?", no es completa. |
> | **Clause** | Grupo de palabras con sujeto y verbo. No toda cláusula es una oración completa. |
> | **Independent clause** | Cláusula que puede existir sola como oración. Equivale a una oración simple. |
> | **Dependent clause** | Cláusula que necesita otra para tener sentido. Siempre va acompañada de una conjunción subordinante. |
> | **Subordinating conjunction** | Palabra que introduce una cláusula dependiente: *because, although, when, if, since, while, after, before, unless*. |
> | **Fragment** | En inglés, una oración incompleta. Es un error gramatical común — escribir una cláusula dependiente sola como si fuera una oración. |

---

**Tags:** #grammar #review #sentences #clauses #independent #dependent #English #IDIG1007 #ESPOL #unit0